...
local function getAttr(inst, name)
	...
	if inst.Parent and inst.Parent:IsA("Model") and inst.Parent ~= Services.Workspace then
		local pok, pval = pcall(inst.Parent.GetAttribute, inst.Parent, name)
...
local function isCrystal(inst)
	...
	if inst:IsA("BasePart") or inst:IsA("Model") then
		if getAttr(inst, "Value") ~= nil then
			result = true
		elseif getAttr(inst, "CrystalName") ~= nil or inst.Name:find("Crystal", 1, true) ~= nil then
			result = true
		end
	end
...
local function instPos(inst)
	...
	elseif inst:IsA("Model") then
		...
		local ok, piv = pcall(inst.GetPivot, inst)
...
local function createEntry(inst)
	local targetAdornee = inst:IsA("BasePart") and inst or (inst:FindFirstChildWhichIsA("BasePart") or inst)
...
local function applyDetails(inst, entry, origin)
	...
	if origin and p then
		distanceText = formatDistance((p - origin).Magnitude)
	end
...
local function updateDistances()
	...
	for inst, entry in pairs(Storage.espCache) do
		if inst.Parent then
			local p = instPos(inst)
			if p then
				local text = formatDistance((p - origin).Magnitude)
...
local function surfaceDistance(part, origin)
	...
	if part:IsA("BasePart") then
		...
		local p = instPos(part)
		return p and (p - origin).Magnitude or math.huge
	end
end
...
local function sortedByScore(scoreFn)
	...
	local function consider(inst)
		if not inst or seen[inst] or not inst.Parent then
			return
		end
...
local function getAimedCrystal()
	...
	local function consider(inst)
		...
		local offset = (p + ESP.offset) - origin
	...
	local function install()
		...
		local function findLoot(free, origin)
			...
			local function consider(inst)
				...
				local distance = (p - origin).Magnitude
